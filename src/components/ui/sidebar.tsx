...
import { siteConfig } from "@/config/site" // Added siteConfig for default title
...
const Sidebar = React.forwardRef<
  ...
  React.ComponentProps<"div"> & {
    ...
    mobileSheetTitle?: string; // Added prop for mobile sheet title
  }
>(
  (
    {
      ...
      mobileSheetTitle, // Consumed prop
      className,
      children,
      ...props
    },
    ...
    const sheetTitleText = mobileSheetTitle || siteConfig.name;
      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            className={cn(
              "w-[--sidebar-width] p-0 flex flex-col", // Ensure p-0 and flex-col
               variant === "sidebar" && "bg-sidebar text-sidebar-foreground",
              "[&>button]:hidden"
            )}
            style={
            ...
            side={side}
          >
            <SheetTitle className="p-4 border-b text-lg font-semibold">
                 {sheetTitleText}
            </SheetTitle>
    ...
    return (
      <div
        ref={ref}
        className={cn("group peer hidden md:block", variant === "sidebar" && "text-sidebar-foreground")}
        data-state={open ? "expanded" : "collapsed"}
        data-collapsible={!open && collapsible === "icon" ? "icon" : collapsible === "offcanvas" && !open ? "offcanvas" : ""}
      ...
      >
        <div
          className={cn(
            ...
            !open && collapsible === "icon" ? (variant === "floating" || variant === "inset" ? "w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4))]" : "w-[--sidebar-width-icon]") : "w-[--sidebar-width]",
        ...
        <div
          className={cn(
            ...
            side === "left"
              ? (!open && collapsible === "offcanvas" ? "left-[calc(var(--sidebar-width)*-1)]" : "left-0")
              : (!open && collapsible === "offcanvas" ? "right-[calc(var(--sidebar-width)*-1)]" : "right-0"),
            !open && collapsible === "icon" ? (variant === "floating" || variant === "inset" ? "w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]" : "w-[--sidebar-width-icon]") : "w-[--sidebar-width]",
        ...
        >
          <div
            ...
            className={cn("flex h-full w-full flex-col",
             variant === "floating" && "rounded-lg border border-border shadow bg-card",
